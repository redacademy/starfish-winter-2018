 ...
 */
  ...
  */
 function starfish_comment_count() {
  ...
  */
 function starfish_entry_footer() {
  ...
  * @return bool
  */
 function starfish_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'starfish_categories' ) ) ) {
 		...
 		set_transient( 'starfish_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so starfish_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so starfish_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in starfish_categorized_blog.
  */
 function starfish_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'starfish_categories' );
 }
 add_action( 'edit_category', 'starfish_category_transient_flusher' );
 add_action( 'save_post',     'starfish_category_transient_flusher' );
  ...
  */
 function starfish_comment_list( $comment, $args, $depth ) {
  ...
  */
 function starfish_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
